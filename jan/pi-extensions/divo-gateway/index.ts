...
export default function divoGatewayExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"For Calendar tools, use args key op, never action. For calendar list/read requests with relative windows like today, tomorrow, this week, or next 7 days, pass explicit ISO startTime and endTime bounds when the calendar tool supports them. Calendar timestamps must include a timezone offset or Z; never send timezone-less values like 2026-07-09T00:00:00. Use half-open local-day ranges: startTime is local start of the first included day, endTime is local start after the last included day. For next 7 days, include today plus the following 6 local days, and make the final answer describe the included dates only.",
