use super::commands::*;
use super::constants::{MESSAGES_FILE, PI_SESSION_FILE, THREADS_DIR, THREADS_FILE};
use super::helpers::{
...
fn test_constants_values() {
    ...
    assert_eq!(PI_SESSION_FILE, "pi-session.jsonl");
}
...
#[test]
fn test_get_pi_session_path_layout() {
    let base = PathBuf::from("/tmp/jandata");
    let path = get_pi_session_path(&base, "tid");
    assert!(path.ends_with("threads/tid/pi-session.jsonl"));
}
