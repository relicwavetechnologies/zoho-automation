  ...
  async preExecute(ctx: AgentHookContext): Promise<AgentHookPreResult> {
    ...
    return {
      additionalSystemPrompt: [
        ...
        '  Example: dataProcessor({ zohoSource: { module: "bills", filters: { date_start: "2025-04-01", date_end: "2026-03-31" } }, script: "...group/filter/aggregate..." })',
        ...
        '- dataProcessor scripts must return an array or object. formatAmount(amount, currency) takes full currency units, NOT cents. formatDate(iso) is also available in the sandbox.',
        '- For CSV exports, set exportCsv=true on the dataProcessor call and provide csvColumns for column ordering.',
        '',
        'FINANCIAL AMOUNT FIELDS (critical for accuracy):',
        '- item._amount and item._total = full document amount (total bill/invoice value).',
        '- item._balance = unpaid/outstanding portion for bills, invoices, and credit notes.',
        '- When the user asks about "outstanding", "unpaid", "overdue amount", or "balance due": use item._balance.',
        '- When the user asks about "total spend", "total bills", or "full amount": use item._amount or item._total.',
        '- For modules without balance (expenses, payments): _balance equals _amount.',
        '- The tool result includes sampleFieldNames — use these to access raw Zoho fields such as vendor_name, due_date, payment_made, and status.',
