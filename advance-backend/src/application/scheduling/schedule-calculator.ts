...
const addMonths = (input: { year: number; month: number }, months: number) => {
  const d = new Date(Date.UTC(input.year, input.month - 1 + months, 1));
...
export const getNextScheduledRunAt = (
  ...
  if (schedule.type === 'hourly') {
    ...
    for (let step = 0; step < 24 * 31; step += 1) {
      ...
      if (Math.abs(step) % schedule.intervalHours === 0) return candidate;
    }
    return null;
  }
  ...
  if (schedule.type === 'daily') {
    for (let offset = 0; offset < 370; offset += 1) {
      ...
      const candidate = zonedDateTimeToUtc({
        ...day, hour: schedule.time.hour, minute: schedule.time.minute, timeZone: schedule.timezone,
      });
  ...
  for (let offset = 0; offset < 36; offset += 1) {
    ...
    const candidate = zonedDateTimeToUtc({
      ...
      hour: schedule.time.hour, minute: schedule.time.minute, timeZone: schedule.timezone,
    });
    if (candidate.getTime() > after.getTime()) return candidate;
  }
