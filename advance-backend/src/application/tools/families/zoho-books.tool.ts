...
export const createZohoBooksTool = (deps: {
  ...
  parameterDocs: [
    ...
    'stage_invoice fields, at minimum: customer_id, date, due_date or payment_terms, and line_items, each carrying item_id or name, quantity, rate, and tax_id. Include place_of_supply whenever the draft carries tax — without it the IGST-versus-CGST check cannot run and only warns that it did not. Every new invoice uses the creation-day date in the Zoho organisation timezone. For a quote or estimate, fileName is required for source verification so the backend explicitly selects the matching saved billing address instead of Zoho defaults. The zoho-books-invoice recipe states the rest.',
