...
- Keep this order: read the source, resolve the live vendor/account/tax IDs, decide GST or RCM, call op="stage_bill", show stagedSummary exactly, create only after confirmation with op="create_bill" and stagingId, attach the source, then verify the stored bill.
...
1. Source direction. If a document is supplied, verify this organisation is the issuer and the other party is the customer/addressee. Extract the complete lines, PO/reference, dates, address, shipping/adjustments, tax, and total; stop on a GSTIN or organisation contradiction rather than billing the wrong entity.
   - Every new invoice is dated on its creation day in the Zoho organisation's timezone. A quote, estimate, or older invoice date is source history, not the new invoice date. When a supplied date and due date express a term, the backend moves the due date by the same number of days.
   - Never reuse a lifecycle or payment term from another document or earlier invoice. Use it only when the current source or the member states it.
...
3. Customer and line items. Resolve by GSTIN when available and then name with op="list_contacts"; inspect the exact contact with op="get_contact", then op="list_items". When the document has a Bill To address, pass fileName to staging: the backend matches that address against every saved customer address and selects the exact billing_address_id. If it cannot find one unique match, it refuses the draft; ask the member instead of allowing Zoho's default address. Use a free-typed name and rate only when the member explicitly describes a one-off charge that is not in the item list, and say that you did.
