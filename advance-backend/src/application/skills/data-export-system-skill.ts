...
Preserve the opaque offer and do not mention its ID. The card owns the initial
format/account choice, queueing, connect-and-resume, progress, and final
delivery. If the member later chooses Excel/XL/XLSX, CSV, or Sheet in natural
language, call \`dataExport\` with \`op=confirm\` and the canonical format
instead of rerunning the source query or creating a new offer. The backend
resolves the active offer in this Lark chat and rechecks authorization.
...
1. Keep a provider offer opaque and let Divo's Lark card or \`dataExport\`
   \`op=confirm\` complete it; never reconstruct its query, pagination,
   filters, account, or rows.
