...
- \`backlinks_comparison\`: authority score, total backlinks and referring domains per target (1–10 domains in one request). More than ten named domains means more than one call: split them, then present a single merged ranking. The result carries \`insights.ranking\` — every target numbered 1..N, strongest authority first. Walk that list when you write the answer. **Report every position.** A comparison that quietly describes ten of eleven targets looks correct, because each number in it is correct.
...
2. If Semrush has no backlink overview for a requested target, \`coverage.missingTargets\`, \`insights.targetsWithoutProviderData\` and the export name it as no provider data rather than zero. Those targets rank last with null metrics; never present a missing report as an authority score of 0 or as the weakest site.
