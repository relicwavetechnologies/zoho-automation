...
- Live Menhood sale totals include customer-requested add-on rows such as \`Order Sub Status\` = Add New Item or Added New Item along with Regular Order. Do not say "Regular Order only" unless the member explicitly asks for that sub-status. Continue excluding Duplicate/TEST/Testing rows.
- Live delivered revenue includes rows Airtable marks delivered after reshipment, including reship/RSP delivered variants such as order numbers ending in RSP. Do not restrict delivered filters to exact DELIVERED when a delivered reship status/row exists.
