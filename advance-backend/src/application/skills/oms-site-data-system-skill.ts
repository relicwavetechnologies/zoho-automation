...
10. Summarize only the useful evidence in chat. The preview contains at most 25 rows. When it contains \`preview.exportOfferId\`, preserve that opaque offer and present its governed Sheet/CSV/XLSX choice once instead of reproducing rows. Do not create a CSV, Excel file, local Python workflow, or temporary download link yourself. If the member later names a format, call \`dataExport\` with \`op=confirm\` for the existing offer rather than rerunning OMS. The exported OMS snapshot may not be exhaustive.
