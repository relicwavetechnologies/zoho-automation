...
function buildCsv<T extends Record<string, unknown>>(
  ...
  const rows = [
    ...
    '',
    `Generated by Divo on ${generatedAt.toLocaleString('en-US', {
      month:      'short',
      day:        'numeric',
      year:       'numeric',
      hour:       '2-digit',
...
function appendEscalation(summary: string, moduleLabel: string): string {
  return [
    summary,
    `Found more ${moduleLabel.toLowerCase()} than can fit inline. Say "export all" for a CSV download, or narrow by date/status.`,
  ].join(' ');
}
...
export async function handleZohoList<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const shouldExhaust = input.exportAll === true || input.narrowFilter === true;
  ...
  if (!shouldExhaust && firstPage.hasMore) {
    const visible = firstItems.slice(0, inlineThreshold);
    const summary = appendEscalation(input.summarize(visible, { truncated: false, hasMore: true }), input.moduleLabel);
    return {
      items: visible,
  ...
  if (!shouldExhaust && firstItems.length <= inlineThreshold && !firstPage.hasMore) {
    return {
      items: firstItems,
      totalCount: firstItems.length,
      summary: input.summarize(firstItems, { truncated: false, hasMore: false }),
      truncated: false,
      hasMore: false,
      suggestExport: false,
    };
