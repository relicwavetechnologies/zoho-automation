...
export const dataExportOfferPayloadSchema = z.object({
  ...
  exportKind: z.enum(['sample', 'full']).optional(),
  rowLimitOverride: z.number().int().positive().optional(),
  sampleOfPlanId: z.string().uuid().optional(),
...
export function parseDataExportOfferPayload(value: unknown): DataExportOfferPayload {
  ...
  return {
    ...
    ...(transform ? { transform } : {}),
    destination,
    ...(parsed.exportKind ? { exportKind: parsed.exportKind } : {}),
    ...(parsed.rowLimitOverride !== undefined ? { rowLimitOverride: parsed.rowLimitOverride } : {}),
    ...(parsed.sampleOfPlanId ? { sampleOfPlanId: parsed.sampleOfPlanId } : {}),
