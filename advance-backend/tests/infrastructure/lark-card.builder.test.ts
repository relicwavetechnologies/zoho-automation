...
describe('lark-card.builder buildStatusCard (layout A)', () => {
  ...
  it('shows todo rail on the right, not a duplicate 2-word summary', () => {
    const payload = buildStatusCard({
      timeline: {
        phase:       'Executing',
        progressPct: 25,
    ...
    const rail = (cols[2]!['elements'] as Array<Record<string, unknown>>)[0]!['content'] as string;
    assert.match(rail, /●.*LARK/i);
    assert.match(rail, /○.*ZOHO/i);
    assert.ok(!/^Updating Lark$/m.test(rail.trim()), 'rail is todos, not duplicate live text');
  });
  ...
  it('omits separate Plan collapsible when plan is in the rail', () => {
    const payload = buildStatusCard({
      timeline: {
        liveLabel: 'Working…',
        plan: [
