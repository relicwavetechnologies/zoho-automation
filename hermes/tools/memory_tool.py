...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 1375, backend=None):
        self.memory_entries: List[str] = []
        self.user_entries: List[str] = []
        # Read-only shared company bucket (enterprise/Postgres only; [] on files).
        self.company_entries: List[str] = []
        self.memory_char_limit = memory_char_limit
        self.user_char_limit = user_char_limit
        # Frozen snapshot for system prompt -- set once at load_from_disk()
        self._system_prompt_snapshot: Dict[str, str] = {"memory": "", "user": "", "company": ""}
        # Persistence backend: Postgres in an enterprise session, files
        # otherwise. Injectable so tests can supply a fake.
        if backend is None:
            from tools.memory_backends import make_memory_backend

            backend = make_memory_backend(
                char_limits={"memory": memory_char_limit, "user": user_char_limit},
                dir_provider=get_memory_dir,
            )
    ...
    def load_from_disk(self):
        ...
        """
        # Persistence is delegated to the backend (files or Postgres).
        # Deduplicate (preserves order, keeps first occurrence).
        self.memory_entries = list(dict.fromkeys(self._backend.load("memory")))
        self.user_entries = list(dict.fromkeys(self._backend.load("user")))
        self.company_entries = list(dict.fromkeys(self._backend.load_company()))

        # Sanitize entries for the system-prompt snapshot only.  Live state
        # (memory_entries / user_entries / company_entries) keeps the raw text
        # so the user can see + remove poisoned entries via the memory tool.
        sanitized_memory = self._sanitize_entries_for_snapshot(self.memory_entries, "MEMORY.md")
        sanitized_user = self._sanitize_entries_for_snapshot(self.user_entries, "USER.md")
        sanitized_company = self._sanitize_entries_for_snapshot(self.company_entries, "COMPANY")

        # Capture frozen snapshot for system prompt injection
        self._system_prompt_snapshot = {
            "memory": self._render_block("memory", sanitized_memory),
            "user": self._render_block("user", sanitized_user),
            "company": self._render_block("company", sanitized_company),
        }
    ...
    def _path_for(target: str) -> Path:
        """Compat: the on-disk path for a target (file backend + drift tests)."""
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        return self._backend.mutate(target, "add", lambda fresh: self._apply_add(target, fresh, content))

    def _apply_add(self, target, fresh, content):
        """Pure add transform over the backend's fresh entry list."""
        ...
        if content in entries:
            return None, self._success_response(target, "Entry already exists (no duplicate added).")
        ...
        return new_entries, self._success_response(target, "Entry added.")
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> Dict[str, Any]:
        ...
        return self._backend.mutate(
            target, "replace", lambda fresh: self._apply_replace(target, fresh, old_text, new_content)
        )

    def _apply_replace(self, target, fresh, old_text, new_content):
        """Pure replace transform over the backend's fresh entry list."""
        self._set_entries(target, fresh)
        entries = list(self._entries_for(target))
        matches = [(i, e) for i, e in enumerate(entries) if old_text in e]

        if not matches:
            return None, {"success": False, "error": f"No entry matched '{old_text}'."}
        ...
        return entries, self._success_response(target, "Entry replaced.")
    ...
    def remove(self, target: str, old_text: str) -> Dict[str, Any]:
        ...
        return self._backend.mutate(target, "remove", lambda fresh: self._apply_remove(target, fresh, old_text))

    def _apply_remove(self, target, fresh, old_text):
        """Pure remove transform over the backend's fresh entry list."""
        self._set_entries(target, fresh)
        entries = list(self._entries_for(target))
        matches = [(i, e) for i, e in enumerate(entries) if old_text in e]
        ...
        entries.pop(idx)
        self._set_entries(target, entries)
        return entries, self._success_response(target, "Entry removed.")
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        # The company bucket is shared + admin-curated; it has no per-user
        # char budget and the agent cannot write to it.
        if target == "company":
            header = "COMPANY KNOWLEDGE (shared, read-only)"
            return f"{separator}\n{header}\n{separator}\n{content}"
