...
export const ControlsPage = () => {
  ...
  const inspectTask = async (taskId: string) => {
    if (!token) return;
    try {
      const result = await api.get<RuntimeTaskDetail>(
        `/api/admin/runtime/tasks/${encodeURIComponent(taskId)}`,
        token,
      );
                ...
                {tasks.map((task) => (
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-zinc-500 mt-1">
                        {task.status} &middot; step: {task.currentStep ?? 'n/a'} &middot; signal: {task.controlSignal}
                      </span>
                    </div>
                  </button>
                ))}
