      ...
      <aside className={cn(
        ...
        <ScrollArea className="flex-1 px-3 py-2">
          <nav className="flex flex-col gap-1">
            {navItems.map((item) => {
              const Icon = iconMap[item.label] || <ChevronRight strokeWidth={1.5} className="h-4 w-4 shrink-0" />;
              ...
              const LinkContent = (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      "flex items-center rounded-md text-[13px] transition-all group relative",
                      isCollapsed ? "justify-center h-9 w-9 p-0" : "gap-3 px-3 py-2",
                      isActive
                        ? 'bg-[#1a1a1a] text-zinc-100 font-medium shadow-sm'
        ...
        <div className={cn("mt-auto pb-4", isCollapsed ? "px-2" : "px-4")}>
          {!isCollapsed ? (
            <div className="rounded-md bg-[#111] p-3 flex flex-col gap-1 border border-[#1a1a1a]">
              <span className="text-xs text-zinc-500">Version</span>
              <Badge variant="secondary" className="bg-[#1a1a1a] text-zinc-400 font-mono text-[10px] w-fit hover:bg-[#1a1a1a] border border-[#222]">
                v0.1.0-beta
              </Badge>
            </div>
          ) : (
            <div className="flex justify-center w-full">
               <Tooltip>
                <TooltipTrigger asChild>
                  <div className="h-[2px] w-4 bg-[#333] rounded-full mx-auto cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="right" className="bg-[#1a1a1a] text-zinc-200 border-[#333]">
                  v0.1.0-beta
                </TooltipContent>
