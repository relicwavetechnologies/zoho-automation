...
async function run() {
  ...
  console.log("\n=== 2. Checking Qdrant for Desktop Channel ===");
  try {
    const qResponse = await fetch(`${QDRANT_URL}/collections/${COLLECTION}/points/scroll`, {
      ...
      body: JSON.stringify({
        filter: {
          must: [
            { key: "channel", match: { value: "desktop" } }
          ]
        },
        limit: 5,
        with_payload: true
    ...
    console.log(`Found ${points.length} points in Qdrant with channel=desktop.`);
    console.dir(points.map(p => ({ id: p.id, ownerUserId: p.payload?.ownerUserId, text: p.payload?.text })), { depth: null });
  } catch (err) {
