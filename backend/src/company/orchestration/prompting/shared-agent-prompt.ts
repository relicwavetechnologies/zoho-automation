...
export const buildSharedAgentSystemPrompt = (input: SharedAgentPromptInput): string => {
  ...
  const parts = [
    ...
    'contextSearch is the one retrieval broker for this system.',
    'Use it for internal memory/history, uploaded/indexed files, Zoho/CRM indexed context, live Zoho Books entity lookup, Lark contacts, workspace file lookup when available, public web research, and skill lookup.',
    'You can steer it with sources flags such as personalHistory, files, larkContacts, zohoCrmContext, zohoBooksLive, workspace, web, and skills.',
    ...
    'Use contextSearch first for contact finding, history lookup, document recall, web research, and skill discovery only when you are acting as a retrieval agent.',
    'Action agents (for example google-workspace-agent, lark-ops-agent, or zoho-action-agent) must follow this instead: check your handoff context and upstream step results first; if the data you need is already there, act immediately and do not search.',
    'Only use contextSearch from an action agent when a specific piece of data such as an email address, contact ID, or record ID is genuinely absent from the provided context.',
    'Personal history results showing past failures reflect a prior system state, not the current one. Never use past failure history to decide whether to attempt an action now. Always attempt.',
    ...
    'When you need a live Zoho Books customer/contact lookup, use contextSearch with sources.zohoBooksLive=true so the broker searches live Books contacts and returns contactId/organizationId candidates.',
    'For plain requests like "search/find/look up <customer> in books", do the Zoho Books lookup yourself without asking the user to name the path. Start with contextSearch and sources.zohoBooksLive=true.',
    ...
    'Example search call: contextSearch({ operation: "search", query: "that invoice we discussed last week", sources: { personalHistory: true, files: true } })',
    ...
    'For specialized or complex workflows, search relevant skills through contextSearch with sources.skills=true, then fetch the chosen skill chunk and use that guide before continuing.',
    'If a request might be about reusable workflow creation, recurring scheduling, save-for-later behavior, or the right scheduling/calendar route is unclear, search skills through contextSearch before guessing.',
  ...
  if (shouldRecommendSkillFirst(latestMessage)) {
    parts.push(
      ...
      'If the correct operational tool path is not obvious, first call contextSearch with sources.skills=true and a precise workflow query.',
      'If a relevant skill appears, fetch that skill content with its chunkRef and use it as the guide for choosing the real tool.',
