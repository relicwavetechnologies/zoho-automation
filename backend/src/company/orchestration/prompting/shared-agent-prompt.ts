...
export const buildSharedAgentSystemPrompt = (input: SharedAgentPromptInput): string => {
  ...
  const parts = [
    ...
    'Use contextSearch first for contact finding, history lookup, document recall, web research, and skill discovery only when you are acting as a retrieval agent. If you are an action agent (for example google-workspace-agent, lark-ops-agent, or zoho-action-agent), check your handoff context first and act immediately when the needed IDs, emails, subject lines, or bodies are already present.',
