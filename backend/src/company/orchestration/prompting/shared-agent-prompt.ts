...
export const buildSharedAgentSystemPrompt = (input: SharedAgentPromptInput): string => {
  ...
  if (shouldPrioritizeInternalDocs(latestMessage)) {
    parts.push(
      ...
      '1. Use indexed company-document and history evidence already provided in your handoff context first.',
      '2. Use documentOcrRead only for exact file extraction, OCR, or verbatim text.',
      '3. If the required document source is still missing, report that gap to the supervisor instead of searching on your own.',
    );
  }
  ...
  if (shouldRecommendSkillFirst(latestMessage)) {
    parts.push(
      ...
      'If the correct operational tool path is not obvious after reviewing your handoff context, report that ambiguity to the supervisor.',
      'Do not guess a workflow or tool route when a supervisor clarification is needed.',
