...
const buildPlannerPrompt = (input: {
  ...
  return JSON.stringify({
    instructions: [
      ...
      '7a. DELIVERY steps — any step whose primary purpose is to send, share, or forward information to a named person ("send to X", "share with X", "message X", "forward to X", "tell X"): NEVER route to context-agent. Route to lark-ops-agent/post_message/lark if the user is in a Lark conversation, or google-workspace-agent/send_email/gmail if email delivery is implied. A step that says "summarize X and send to Y" is a DELIVERY step, not a lookup step.',
      ...
      '- context-agent may ONLY use action=cross_source_lookup and sourceSystem=context. It CANNOT send, post, message, deliver, or perform any write/mutation. If a step needs to deliver information to a person, use a different agent.',
      ...
      '- "search for best restaurants and send summary to Anish" => step_1 context-agent/cross_source_lookup/context (web search), step_2 lark-ops-agent/post_message/lark (or google-workspace-agent/send_email/gmail) dependsOn step_1',
