...
import { resolveToolPolicyDescriptor } from '../vercel/runtime-approvals';
...
import { type DesktopChildRoute } from '../core/child-router';
...
import { executeSharedSupervisorTask } from '../core/supervisor-runtime';
import {
...
const requiresManagerApproval = (
  ...
  && (() => {
    const policy = resolveToolPolicyDescriptor(pendingApproval.toolId);
    const actionGroup = pendingApproval.actionGroup;
    if (!actionGroup || !policy.overrideableApprovalActionGroups.includes(actionGroup)) {
      return false;
    }
    return runtime.departmentManagerApprovalConfig!.requiredToolIds.includes(
