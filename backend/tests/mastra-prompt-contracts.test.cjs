...
test('Mastra supervisor prompt is branded as Odin and keeps hard grounding rules', () => {
  const source = readSource('company/integrations/mastra/agents/supervisor.agent.ts');
  assert.match(source, /name: 'Odin'/);
...
test('Mastra search prompt enforces short grounded outputs', () => {
  ...
  assert.match(source, /name: 'Odin Search'/);
...
test('Mastra ack prompt keeps the terse Odin budget', () => {
  const source = readSource('company/integrations/mastra/agents/ack.agent.ts');
  assert.match(source, /Odin AI/i);
