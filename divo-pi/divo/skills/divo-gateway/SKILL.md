...
- For a normal public lookup, comparison, pricing check, verification, or current-facts question, prefer loading the exact web-search skill named in the injected catalogue once when it has not already been loaded, then invoke `tools.invoke` with payload `{ "toolId": "webSearch", "args": { "query": "<focused query>", "limit": 5 } }`. If the recipe is unavailable, continue with the clear permitted direct capability instead of treating missing guidance as permission denial. You still do not need `divo_skill_resolve`, `capabilities.get`, or `tools.list` — the catalogue already identifies the capability.
