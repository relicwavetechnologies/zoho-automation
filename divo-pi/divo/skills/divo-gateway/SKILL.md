...
3. Run the file with `bash` and `python3`. Connected calls use exactly `divo-local invoke --tool <toolId> --args-file <path>`. The client automatically saves each successful governed envelope to a new protected JSON file inside `DIVO_RUN_DIR` and prints only its path, byte count, and trace. Read that returned path in Python. Any required provider schema `describe` also runs once inside this same file through `divo-local`; never call the registered provider tool first and then rediscover the same schema in Python. The provider result is under `data`; never count keys in that object as records. Use the source skill's exact row, count, and pagination fields, and print only counts or aggregates, never rows.
