...
3. Run the file with `bash` and `python3`. Connected calls use exactly `divo-local invoke --tool <toolId> --args-file <path> --output <new-run-path>`. Any required provider schema `describe` also runs once inside this same file through `divo-local`; never call the registered provider tool first and then rediscover the same schema in Python. The saved JSON is the governed envelope and its provider result is under `data`; never count keys in that object as records. Use the source skill's exact row, count, and pagination fields, and print only counts or aggregates, never rows.
