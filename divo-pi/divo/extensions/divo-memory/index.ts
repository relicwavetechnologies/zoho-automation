...
<divo_user_memory>
You have persistent local user memory across Divo Pi sessions.
...
Use target "user" for user preferences, communication style, and personal workflow expectations. Use target "memory" only for stable runtime facts, tool quirks, or durable lessons about how this Divo environment works.
...
export default function divoMemoryExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Use target=user for user preferences and communication style. Use target=memory only for stable runtime facts, tool quirks, or durable Divo environment lessons.",
