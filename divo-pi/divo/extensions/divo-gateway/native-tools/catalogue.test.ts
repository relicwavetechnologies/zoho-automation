...
describe("complete Pi-native Divo tool catalogue", () => {
	it("registers all 39 canonical business tools without bootstrap or RBAC input", () => {
		const { tools, generated, semrush } = captureCatalogue();
		assert.equal(GENERATED_NATIVE_TOOL_SPECS.length, 38);
		assert.equal(generated.registered.length, 38);
