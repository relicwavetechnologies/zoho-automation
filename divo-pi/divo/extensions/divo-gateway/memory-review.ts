...
export function registerMemoryReviewTool(
	...
	pi.registerTool({
		...
		description:
			"Review durable department or company facts against the authenticated runtime department and publish only the exact approved selection. When the user explicitly names team/department or company scope, pass that requestedScope. Personal preferences use the private background-learning path without review; never claim their persistence in the same reply.",
		promptSnippet:
