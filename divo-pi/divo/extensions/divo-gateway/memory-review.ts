...
export function registerMemoryReviewTool(pi: ExtensionAPI) {
	pi.registerTool({
		...
		description:
			"Recheck backend memory authority using the authenticated runtime department, show the review card for durable proposed facts, and publish only the exact approved selection. Pass only proposalId and bullets; this tool fetches canonical targets itself.",
		promptSnippet:
