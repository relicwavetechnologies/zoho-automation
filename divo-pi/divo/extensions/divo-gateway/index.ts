...
3. Run that file with Bash using python3 and a specific visible description. Connected calls use exactly the divo-local client command divo-local invoke --tool <toolId> --args-file <path> --output <new-run-path>, where the output is a new path inside DIVO_RUN_DIR; never print or cat rows. Any required provider schema describe also runs once inside this same file through divo-local; never call the registered provider tool first and then rediscover the same schema in Python. The saved JSON is the governed envelope; the provider result is under data. Never count keys in that object as records: use the source skill's exact row, count, and pagination fields. Parse only in Python and print counts or aggregates. Never supply skillId: the runtime attaches trusted provenance. Never use curl, raw backend URLs, member tokens, or SaaS credentials.
