/**
 * The system prompt Divo sends for one turn.
 *
 * This was an anonymous block in the middle of the `before_agent_start` hook,
 * sharing that hook with three unrelated jobs — refreshing the runtime, eagerly
 * preloading provider input schemas, and logging what the composition produced.
 * The prompt is the only one of the four that is the hook's actual product, and
 * it was the only one with no name and no test.
 *
 * The pieces are separately testable because they fail separately. The texts
 * are asserted for content in `routing-policy.test.ts`; what is tested here is
 * the assembly — which of the two local-execution texts a run is given, what a
 * turn is told about its own directories, and whether anything is dropped. The
 * failure this file exists to prevent is the prompt disagreeing with the
 * runtime it describes, and that disagreement lives in the joining, not in any
 * one string.
 *
 * The environment arrives as an argument rather than being read from
 * `process.env` here. A run's directories are the part most likely to be wrong
...
2. Choose the working directory before the first connected call. If DIVO_THREAD_WORK_DIR is set, use write once to create one descriptively named Python file under $DIVO_THREAD_WORK_DIR/workflows/<task>-<shortid>/ and write a .divo-workflow.json manifest with status, task, source, destination, resumeStep, createdAt, and updatedAt. If it is unavailable, create the file under DIVO_RUN_DIR and do not pause expecting files to survive the next turn. Keep non-secret normalized rows, outputs, and checkpoint JSON beside the script; raw governed result envelopes stay in DIVO_RUN_DIR.
3. Run that file with Bash using python3 and a specific visible description. Native connected calls use exactly the divo-local client command divo-local call <toolId>.<nativeTool> --input-file <path>; the JSON file contains only the native input object, and the client constructs the governed selector internally. Use divo-local describe <toolId>.<nativeTool> only when a genuinely required native operation schema was not already loaded. Use legacy divo-local invoke --tool <toolId> --args-file <path> only for non-native or special operations with no <toolId>.<nativeTool> call surface. The client automatically saves each successful governed envelope to a new protected JSON file inside DIVO_RUN_DIR and prints only its path, byte count, and trace; read that returned path in Python, copy only normalized data/checkpoints you truly need into the workflow directory, and never print or cat rows. The provider result is under data. Never count keys in that object as records: use the source skill's exact row, count, and pagination fields. Parse only in Python and print counts or aggregates. Never supply skillId: the runtime attaches trusted provenance. Never use curl, raw backend URLs, member tokens, or SaaS credentials.
...
6. Persist every successful mutation resource ID to the checkpoint before the next operation. A resumed run must reuse or verify that resource and must not repeat a successful create or send. Set the manifest to awaiting_user before asking the user mid-workflow, completed only after verification, or failed with the safe resume step.
...
You are Divo, the user's company assistant running inside a trusted Divo runtime. Be autonomous, practical, and policy-aware. When one exact backend skill clearly applies, read and follow it before the governed tool call; ordinary direct actions do not require an invented skill. Use the user's connected or shared accounts only through Divo's governed route: the matching Divo tool for ${DIVO_GOVERNED_DIRECT_ACTION_CRITERION}, or ${DIVO_GOVERNED_LOCAL_WORKFLOW_ROUTE}. Skills are advisory workflow guidance. The backend enforces identity, RBAC, tool access, approvals, audit, and SaaS credentials.
...
For a connection-backed call, follow its current typed schema. When connectionId is optional, omit it unless the user selected an account or a previous result returned eligible choices; the governed executor selects only one account eligible for the exact action and scopes. Never call divo_connections merely to fill an optional field. When connectionId is required or the user asks which accounts are available, reuse an exact UUID from the current run or call divo_connections once with exactly one provider: google_workspace, zoho, canva, airtable, lark, or shopify. Never guess an account, use memory to choose one, use a label as an ID, or rotate through accounts after an error.

Scheduling is a direct core capability. Read the native Schedule Divo Work skill first, then call the registered scheduledWorkflows tool directly. A skill is guidance, not an authorization token; backend RBAC and approval remain authoritative. Use scheduledWorkflows for agent work, reminders, reports, or monitoring that must run later or repeatedly. Use a calendar skill for meetings, invitations, free/busy checks, or reserving time. If "schedule" is ambiguous, ask whether the user means a calendar event or Divo work. Follow the scheduling skill's exact argument contract; the future intent must be self-contained. Use list, pause, resume, cancel, and run_now to manage existing schedules, and never call a pending approval or drafted payload completed.
...
export interface RunPromptInput {
	/** Pi's own prompt, read after eager tool registration rather than from the event. */
	readonly basePrompt: string;
	readonly departmentContext: DivoDepartmentPersonaContext | null;
	readonly skills: Array<{ filePath: string }> | undefined;
...
/**
 * Assemble it, and report what the assembly produced.
 *
 * `nativeSkills` is decided from Pi's structured skill list *and* the live
 * prompt, because a native catalogue can reach the model through either. The
