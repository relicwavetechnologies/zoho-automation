...
import { X, FileText, Image as ImageIcon, File, Clock, Shield, Trash2, Loader2, RotateCcw, Share2, Search } from 'lucide-react'
...
function FileTypeIcon({ mimeType }: { mimeType: string }) {
  if (mimeType.startsWith('image/')) return <ImageIcon size={14} className="text-primary/60" />
  if (mimeType === 'application/pdf') return <FileText size={14} className="text-red-500/50" />
...
function shareLabel(status?: string, sharedCompanyWide?: boolean) {
  ...
  if (status === 'pending') return { text: 'Pending approval', cls: 'text-amber-500/60' }
  if (status === 'delivery_failed') return { text: 'Delivery failed', cls: 'text-orange-500/60' }
  if (status === 'reverted') return { text: 'Reverted', cls: 'text-muted-foreground/40' }
  return null
}
    ...
    <>
      ...
      <div
        className={cn(
          ...
          open ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-[0.98] pointer-events-none',
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/50">
          <div className="flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/80">Files</span>
            <span className="text-[13px] font-bold text-foreground/90">Asset Library</span>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="overflow-y-auto flex-1 px-5 pb-5 space-y-1">
          {loading && (
            <div className="space-y-4 py-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-4 px-3 py-2 border border-transparent">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  <p className="text-[13px] font-bold text-foreground/90 truncate">{file.fileName}</p>
                  <div className="flex items-center gap-2 mt-1 whitespace-nowrap overflow-hidden text-[11px] font-medium text-muted-foreground/50">
                    <span className={cn('font-bold', label.cls)}>{label.text}</span>
                    <span className="opacity-30">·</span>
                    <span>{formatBytes(file.sizeBytes)}</span>
                    <span className="opacity-30">·</span>
                    <span className="flex items-center gap-1">
                      <Clock size={10} className="opacity-60" /> {formatDate(file.createdAt)}
                    </span>
                  </div>
                  ...
                  {(file.accessPolicies.length > 0 || share) && (
                    <div className="flex items-center gap-2 mt-1.5">
                      {file.accessPolicies.map((p, idx) => (
                         <span key={idx} className="bg-primary/10 border border-primary/20 text-[10px] font-bold text-primary/70 px-1.5 py-0.5 rounded-lg truncate">
                           {p.aiRole}
                         </span>
                      ))}
                      {share && (
                ...
                <div className="flex items-center gap-1 ml-2">
                  ...
                  <div className="flex items-center gap-1 border-l border-border/50 ml-2 pl-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {file.ingestionStatus === 'failed' && (
                      ...
                      >
                        {retryingIds.has(file.id) ? <Loader2 size={14} className="animate-spin" /> : <RotateCcw size={14} />}
                      </button>
                    )}
                    ...
                    {canTriggerShare && (
                      ...
                      >
                        {sharingIds.has(file.id) ? <Loader2 size={14} className="animate-spin" /> : <Share2 size={14} />}
                      </button>
                    )}
                    ...
                    <button
                      ...
                      title="Delete File"
                    >
                      {deletingIds.has(file.id) ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}
                    </button>
                  </div>
                </div>
              </div>
