...
const TOKEN_KEY = "divo_desktop_token";
const departmentStorageKey = (companyId: string) =>
  `divo_desktop_department_${companyId}`;

const resolveSelectedDepartmentId = (
  ...
  const stored = localStorage.getItem(
    departmentStorageKey(nextSession.companyId),
  );
...
export function AuthProvider({
  ...
  useEffect(() => {
    const unsub = window.desktopAPI.auth.onCallback(
        ...
        try {
          ...
          if (res.success && res.data) {
            ...
            setSelectedDepartmentIdState(
              resolveSelectedDepartmentId(newSession),
            );
  ...
  useEffect(() => {
    ...
    if (selectedDepartmentId) {
      localStorage.setItem(
        departmentStorageKey(session.companyId),
