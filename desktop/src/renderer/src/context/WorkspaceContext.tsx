...
const CURRENT_WORKSPACE_KEY = 'cursorr_desktop_workspace_current'
const RECENT_WORKSPACES_KEY = 'cursorr_desktop_workspace_recent'
const THREAD_WORKSPACE_KEY = 'cursorr_desktop_workspace_threads'
...
export function WorkspaceProvider({ children }: { children: ReactNode }): JSX.Element {
  ...
  useEffect(() => {
    localStorage.setItem(THREAD_WORKSPACE_KEY, JSON.stringify(threadWorkspaceMap))
  }, [threadWorkspaceMap])

  const currentWorkspace = useMemo(
    () => recentWorkspaces.find((workspace) => workspace.id === currentWorkspaceId) ?? null,
    [currentWorkspaceId, recentWorkspaces],
  )

  const selectWorkspace = useCallback(async (): Promise<WorkspaceFolder | null> => {
  ...
  const bindThreadToCurrentWorkspace = useCallback((threadId: string) => {
    if (!currentWorkspace) return
    setThreadWorkspaceMap((prev) => ({ ...prev, [threadId]: currentWorkspace.id }))
  }, [currentWorkspace])
  ...
  const getThreadWorkspace = useCallback((threadId: string): WorkspaceFolder | null => {
    const workspaceId = threadWorkspaceMap[threadId]
    if (!workspaceId) return null
    return recentWorkspaces.find((workspace) => workspace.id === workspaceId) ?? null
  }, [recentWorkspaces, threadWorkspaceMap])

  const isThreadInCurrentWorkspace = useCallback((threadId: string): boolean => {
