...
| VERIFY-001 | `googleSheets` | High | Main | A 3,467/1,294-row write was “verified” with only 50 rows returned | The read operation's default/cap was treated as complete verification and the script did not fail the mismatch | Document/use paged or exact-range count verification and make the shared helper raise on mismatch | Pending | Open |
