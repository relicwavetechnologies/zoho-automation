...
**T8 — Substring matching is literal and undocumented.** ✅ **fixed 2026-08-03.** `String.includes` on lowercased values, so `subjectContains: "OTP|verification code"` or `"*invoice*"` matched nothing while `list` reported the rule valid — silent for weeks, the defect class this whole plan is about. Three cases, three answers:

- **Decoration is stripped mechanically.** Surrounding quotes (straight or curly), leading/trailing `*` and `%`, and collapsed whitespace. Nobody writing `*invoice*` wants a subject with asterisks in it, and none of these changes *which* messages match.
- **Alternation is supported properly.** `subjectContains`, `bodyContains` and `notSubjectContains` now take a list — `["OTP", "verification code"]` — matching when any phrase does. Stored sorted and de-duplicated, and folded the same way into `mailRuleDedupeKey`, so the same set in two orders is one rule rather than two forwarding every message twice; a single phrase and a one-item list are also the same rule, so adding an alternative and removing it again returns the member to where they started.
- **A pattern is refused by name.** `|`, an interior wildcard, or regex syntax gets a refusal naming the fix. `|` is deliberately **not** split: `Acme | Invoice 42` is an ordinary subject line, so splitting would silently *widen* somebody's rule — the same defect as the silent narrowing it replaces, pointed the other way.

The refusals apply to **creation only**. A stored rule is normalised but never refused on read, because a rule whose subject genuinely contains a `|` has been working, and stopping it would be a worse failure than the one being fixed. The contradiction check (`subjectContains` vs `notSubjectContains`) reads lists too, and calls a rule dead only when *every* alternative is excluded.
...
**Ceiling — fixed in Wave 10.** The source is fetched with `format=raw`, and posting it as JSON base64-encodes it again, so the *encoding* was the limit rather than the size: a forward carrying a 4 MB attachment exceeded the roughly 5 MB draft cap and failed outright. Above 3 MiB the draft now goes through `uploadType=media` as `message/rfc822` bytes; past Gmail's own 25 MB cap the client refuses by name and the delivery is abandoned once rather than retried five times into the same wall.

**Wave 9 — Code quality** *(§3)* — ✅ done, then cold-reviewed. **Round two found two regressions this scope introduced, both of them the failure this whole exercise is about — a promise the runtime declines to keep:**

- **Every `403` that was not a rate limit became `scope_missing`.** The commit's own argument is that a `403` is ambiguous and the machine-readable reason is what separates the meanings — and then it used the reason for one branch only. Gmail's commonest `403` on `users.watch` is `reason: forbidden`, "User not authorized to perform this action", raised when the **Divo-owned** Pub/Sub topic is missing its publisher binding. That put *"Reconnect Google"* in front of every affected member — a thing they can do nothing with, hiding the operator who could actually fix it. Worse than the substring pass it replaced, which filed that error correctly. `scope_missing` is now an allow-list of named scope reasons; an unnamed `403` says nothing rather than something untrue.
- **The hourly ceiling silently stopped capping.** Measuring the window on arrival (the round-one fix) made the count depend on the batch being walked oldest-first, and nothing established that: `recordEvents` read with no `orderBy`, and the natural order of its `IN (...)` lookup is by message ID. Newest-first, every message in a recovered backlog sees an empty window and the entire flood goes out. Fixed in both places — the query orders, and the loop that depends on it sorts, so a future change to how events come back cannot quietly uncap every rate-limited rule. The same finding's root cause covered the half-open upper bound: Gmail's `internalDate` routinely carries only second precision, so a mailing-list burst shares one `occurredAt` and, excluded from each other's windows, none of it counted. Now inclusive, with the message excluded by identity instead.

Also from round two: the retry-ladder literal recorded above; a comment in `recipientMatches` still denying the widening its own code had just performed; and two claims in the handover — `ambiguous` "never implemented", and the `rfc822msgid` search described as current — both describing a subsystem that no longer exists. `countRecentDeliveries`, the one query on the loss-bearing path, had **no repository test at all**; its window, its exclusions and its status filter are all pinned now.

**Round three: `ship`.** Its one substantive finding was a test of mine that proved nothing — the burst case was asserted through a worker fake that implemented the inclusive bound itself, so it passed with the repository predicate reverted. Deleted rather than repaired: the repository test already pins that window on the real query shape, and a test that cannot fail is worse than no test, because it reads as coverage. Round three also found that entries in `SCOPE_REASONS` were unreachable — Google puts `ACCESS_TOKEN_SCOPE_INSUFFICIENT` in `error.details[]`, which nothing read — so a genuine scope loss arriving with `errors[0].reason: forbidden` would have been filed as an operator problem and the member never told to reconnect. That is the round-two fix overshooting, and it is now read most-precise-first.

**Round four: `ship`.** It went looking for a fourth wrong direction in the same classifier and found none — it walked nine real Google error shapes through the shipped client and confirmed every one classifies as it did before, except the scope case that was the point. What it did find is that the round-three repair landed for one spelling of the reason and not the other: case folding alone matches `ACCESS_TOKEN_SCOPE_INSUFFICIENT` but not `accessTokenScopeInsufficient`, and never `insufficient_scope`. Both sets now compare on a separator-stripped fold, so one reason is one entry. It also surfaced a falsehood this wave introduced and nobody had caught: `backendError` sat in the rate-limit set, so a Gmail outage told the member *"Google is rate-limiting this mailbox"* — the advice that followed was right, and the sentence was not, which sends somebody hunting a quota they never exceeded. A Google-side failure now says so in its own words under `provider_unavailable`.

**Wave 10 — Retention and operations** — mostly done.

- ✅ **Pause now stops mail already reserved.** Reserving a delivery and sending it are separate stages that can be over a minute apart once a failed attempt is on the retry ladder, and the rule's status was read only in the first — so a member who paused a rule watched it keep forwarding. The rule is asked again before the send, placed *after* the draft probe: a retry holding a staged draft may be retrying a send that already succeeded, and abandoning it there would file a lie about mail already in somebody's inbox.
- ✅ **A stalled backlog can drain.** The old truncation guessed the cursor forward to the last record consumed, which made progress only when a pass consumed something — ten pages of history Divo does not care about advanced nothing, so the next pass read the same ten and failed identically, forever. Gmail's `pageToken` is persisted on `MailboxSubscription` instead. Progress rides on the token and the cursor is **held still** while one is outstanding, because a token is only valid against the `startHistoryId` it was issued under; the cursor moves once, when the walk finishes and the token clears. `history_backlog_stalled` survives as the last resort for a truncation returning no token. Schema pushed to `divo_dev`, re-diff empty.
- ✅ **Retention.** Bodies stripped from events at 30 days, terminal delivery payloads dropped at 30, events deleted at 90 — three ages because they answer three questions. The body is the sensitive part and nothing reads it after arrival; the *event* is what stops a re-delivery, so it has to outlive any plausible replay; the frozen payload is a second copy of the body needed only while a retry is possible. An event with a delivery still `pending` or `sending` is never taken at any age: the delivery cascades with it, and a 90-day-old pending delivery means something else is wrong that losing the evidence would not fix. The sweep runs at the end of a tick, at most hourly, and its failures are logged and swallowed — a retention sweep that could not run is not a reason to stop delivering mail. **Nothing was ever deleted before this**: a mailbox watched for a year held a year of message bodies, in two places.
- ✅ **Operator-triggered reconciliation.** `POST /api/mail-automations/reconcile` brings the member's active mailboxes forward to be polled now. It moves the schedule and nothing else: it does not clear a failure code, because whether the failure is over is the next pass's answer to give; and it will not wake a paused mailbox, because that is a decision somebody made and this is the wrong door to reverse it through.
- ✅ **The `MailboxReconciliation` audit row.** A recovery means the cursor was rejected outright, so mail was missed and swept back in by date rather than by history; `truncated` marks the only place in this system where mail is knowingly lost. That was a log line and nothing else, and a log line is not something anybody can query a month later when they are asked what happened to a message. Written best-effort: it is evidence about a sync, not part of one, and failing a pass over an audit insert would turn a recovered mailbox back into a stopped one. Schema pushed to `divo_dev`, re-diff empty.
- ✅ **Metrics.** A `mail_ops.tick_summary` line per wake-up — watches renewed, mailboxes synced and failed, deliveries attempted, sent and failed, with the duration. Counters and a log line rather than a metrics client, because there is no backend to send them to and an abstraction over a thing that does not exist is worse than the thing. Emitted only when the tick did something; a worker waking every ten seconds would otherwise bury the interesting ticks under eight thousand a day that say nothing. A failed mailbox is decremented from `mailboxesSynced` — a summary reading "twenty synced, three failed, of twenty" is arithmetic nobody checks and everybody misreads.
- ✅ **The large-attachment forward ceiling.** The ordinary draft endpoint takes the whole message base64-encoded inside a JSON body, which inflates it by a third and is capped near 5 MB — so **the encoding was the ceiling, not the size**, and a forward carrying a 4 MB attachment failed outright while the member saw a rule that had worked all week stop for one message. Above 3 MiB raw the draft now goes through `uploadType=media` as `message/rfc822` bytes. One request, not a resumable session: media upload carries a whole message to Gmail's own size cap, and nothing larger can be sent at all, so there is no range resumable would reach that this does not. Past that cap the client raises `MailTooLargeError` and the worker abandons the delivery **once, in words** — left on the retry ladder it burned five attempts and ended abandoned anyway with a `lastError` that read like a transient provider fault, with nothing saying the message was the reason.

**Wave 10 is complete**, after a cold review that found four things worth fixing — three of them defects this wave introduced.

- **Retention silently broke the dry run.** Both metadata readers required `bodyText` to be a string, and retention removes exactly that key. A stripped event became unreadable, dropped out of matching entirely, and was *still counted* as considered — so testing a rule against older mail answered "we looked at 200 messages and yours caught none" when the rule was correct and the body was simply gone. That is the dry run recreating the failure it exists to remove. Both readers tolerate a missing body now; `consideredCount` counts only what was read; and a rule matching on `bodyContains` reports `bodyUnavailableCount` rather than calling an unanswerable question a non-match.
- **The sweeps were unbounded.** The first sweep after this ships meets everything ever recorded — a year of events in one `DELETE`, cascading to their deliveries — and it is awaited inside a tick that is re-entrancy guarded, so it would block every delivery for as long as it ran. That is the exact guarantee the sweep is documented to keep, broken by the sweep. Worse, a statement large enough to hit a statement timeout would fail, be retried identically an hour later, and never once make progress. Batched at 1,000 rows, ten batches a sweep, stopping the moment a batch comes back short. Indexes added on `MailEvent.occurredAt` and `MailDelivery(status, updatedAt)`, which the sweeps would otherwise sequentially scan hourly, forever.
- **A dead page token wedged the mailbox permanently.** Nothing clears `historyPageToken` on a failed pass, so a token Gmail no longer honours — days old because the mailbox was paused or the process was down — would be resent on every subsequent pass and fail identically. The page-token mechanism reintroducing its own permanent wedge, one step along. A non-404 4xx on a pass carrying a token now restarts the walk once without it, which is safe *only* because of this wave's own invariant: the cursor never moved while the token was outstanding.
- Plus a dead local, a docblock still describing the protocol this wave replaced, and two plan lines asserting behaviour the code does not have — including one claiming the rate cap defers rather than drops, which the same document contradicts forty lines earlier.

The reviewer separately confirmed by mutation that every test written for this wave fails when its production hunk is reverted.
...
| D7 | Stale cursor → `MailboxReconciliation` row with correct window; rate cap **drops and records a `blocked` row** — deferring would hold a flood back for an hour and then release all of it at once, which is the outcome a ceiling exists to prevent |
