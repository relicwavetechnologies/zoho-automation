...
**D11.** `to` matches the union of `To`, `Cc`, `Bcc` and `Delivered-To`. Being copied is not a different event to the person receiving the mail, and `Delivered-To` is the only header that survives an alias or group expansion — the case where the address the user actually typed appears nowhere else in the message. Recipients are compared as whole mailboxes per DR-8, so `ana@example.com` no longer matches `dana@example.com`, and a display name claiming to be an address (`"ana@example.com" <impostor@evil.example>`) is read as the bracketed mailbox, the same rule `from` already followed. That holds for every construct the display position allows — a quoted name, a parenthesised comment, an encoded word — because each may legally hold a comma, and splitting the header through one leaves a fragment that reads as the address it imitates, which would let any sender fire someone else's rule at will simply by naming themselves after it. `MailMessageMetadata` gained optional `cc` / `bcc` / `deliveredTo`; events recorded before this carry `to` alone and match exactly as they used to.
...
Two changes do reach stored rules, and are meant to. `hasAttachment` narrowed for every rule, so one that used to fire on a signature logo now does not — that was the defect, and leaving old rules on the old reading would mean two answers to the same question. And a repeated `To` header is now read whole rather than last-wins, so a free-text `to` rule tests against marginally more text; repeated `To` is rare enough that the alternative — keeping a bug for compatibility with itself — is worse. Note also that `update` submits the whole rule through the strict schema, so a rule in either legacy shape cannot be renamed without being rewritten into the current one. Nothing new can be created in either shape. This departs from DR-8's "flagged in the health API": flagging them `valid: false` would be a lie, since they do match mail. They are simply the last of their kind.
