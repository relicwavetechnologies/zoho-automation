...
- ⏳ **Metrics** and the `MailboxReconciliation` audit row — the audit row needs a model, so it carries a schema change.
- ⏳ **The large-attachment forward ceiling.** `format=raw` plus JSON can exceed Gmail's non-resumable draft limit, so a big attachment fails the forward outright today. Resumable upload is the fix. This is the one remaining item that is a *functional* gap rather than an operational one.
