...
## Task
- 11 - Observability Retries Error Taxonomy

## Objective
- Implement consistent error classification, bounded retries, and structured runtime logs.
...
- Focus on predictable failure handling and debugging visibility.
...
- Primary DTO impact: ErrorDTO classification and retriable behavior, AgentResultDTO retry metadata.
...
- Retry policy must be deterministic and capped; terminal failure must emit classified error.

## Expected Code Touchpoints
- backend/src/engine/observability, backend/src/engine/orchestration, backend/src/engine/agents

## Execution Steps
- Add classifier utility; add retry helper; emit structured logs for route, dispatch, retry, completion.

## Validation
- Each error class maps correctly; retry count does not exceed policy ceiling.

## Definition Of Done
- Operational traces are sufficient to diagnose failed flows.
- todo.md updated with owner and final status for every item.
- progress.md contains exact changes, blockers, and next step.

## Non-Goals
- Do not attempt full enterprise monitoring rollout in this task.
